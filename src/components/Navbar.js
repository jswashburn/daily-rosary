    ...
    return (
        <div className="navbar clr--basic">
            ...
            <ul className="navbar__days-list">
                ...
                {DAYS.map(d => <NavbarDayLink onClick={toggleActiveDay} key={d} dayOfWeek={d} isCurrDay={currDay === d} isSelected={selectedDay === d}/>)}
            </ul>
        </div>
    )
}
...
function NavbarDayLink(props) {
    ...
    const currDay = props.isCurrDay ? ` navbar__item--${props.dayOfWeek.toLowerCase()}` : "";
    return (
        <li>
            <Link
